...
def get_data(file_path, shape=False, file_names=False):# todo don't think I need this but check anyway and look at notes
    ...
    numpy array or tuple:
            If shape = False, file_names = False: Returns numpy array containing data from tiff files as a
                        n x shape x shape (shape refers to the dimensions of the TIFF file, typically 1024 x 1024).
            If shape = True: returns tuple. First element is a numpy array containing data from tiff files as an
                        n x shape x shape (shape refers to the dimensions of the TIFF file, typically 1024 x 1024).
                        Second element is the dimensions of the data array as a tuple.
    ...
    Notes:
        - All images must be of same dimension. If images of different shapes needed, either group them by shape and
            run this function for each file shape or create it manually by reading files into a numpy array with
            dtype = object. Be careful as this can lead to later problems
...
def get_stage_position(file_names, idx_start, idx_end, unique_positions=False):  # todo may not be needed. Check
    """
    ...
    >>> get_stage_position(file_names, 9, 11)
        [10. 20. 40. 30. 40.]

    >>> get_stage_position(file_names, 9, 11, unique_positions = True)
...
def _clean_and_filter(data_array_1d):
    ...
    Need to consider the box size for these filters. Additionally, issues can arise if the data array has nan values.
...
def rmv_xrays_all(data_array, plot=True, std_factor=3):  # todo update docstring
...
def power_fit(data_array, x_vals, return_baseline=False):  # todo add docstring and optimize
...
def fit_high_s(data_array, x_vals, s_range, return_baseline=False):  # todo add docstring and optimize
    if len(data_array.shape) == 2:
        corrected_data = []
        baseline = []
        for i in range(len(data_array)):
...
def remove_radial_outliers(image, center):
...
def cleaning_2d_data(data_array, center, correct_factor=3):
    xlength, rmat = preprocess_for_azimuthal_checking(data_array, center)
    res2d = np.copy(data_array)

    mask_detect = True
    for i in range(xlength):
        roi = np.copy(data_array[rmat == int(i + 1)])
        if len(roi) == 0:
            break
...
def outlier_rev_algo(data_array_1d, correct_factor=3):
    index = np.logical_or(data_array_1d >= np.nanmean(data_array_1d) + correct_factor * np.nanstd(data_array_1d),
                          data_array_1d <= np.nanmean(data_array_1d) - correct_factor * np.nanstd(data_array_1d))
    data_array_1d[index] = np.nan
    return data_array_1d
